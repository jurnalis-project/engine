...
use character::feat::{FeatCategory, FeatDef, FeatEffect};
use character::{
                ...
                || matches!(state.game_phase, GamePhase::Exploration) =>
        {
            match resolve_numeric_selection(input, &pd.candidates) {
                Some(name) => {
                    ...
                    let parts = [
                        pd.verb_prefix.as_str(),
                        name.as_str(),
                        pd.verb_suffix.as_str(),
                    ];
                    parts
                        .iter()
                        .filter(|p| !p.is_empty())
    ...
    match step {
            ...
            let selected = input
                ...
                .or_else(|| {
                    all_races
                        .iter()
                        .copied()
                        .find(|r| r.to_string().to_lowercase() == input_lower)
                });
            ...
            let selected = input
                ...
                .or_else(|| {
                    options
                        .iter()
                        .copied()
                        .find(|o| o.to_lowercase() == input_lower)
                });
        ...
        CreationStep::ChooseClass => {
            ...
            let selected = input
                ...
                .ok()
                .and_then(|n| {
                    if (1..=all_classes.len()).contains(&n) {
                        Some(all_classes[n - 1])
                    } else {
                        None
                    }
                })
                .or_else(|| {
                    all_classes
                        .iter()
                        .copied()
                        .find(|c| c.to_string().to_lowercase() == input_lower)
                });
        ...
        CreationStep::ChooseWizardSpellbook => {
            ...
            let selected =
                match parse_numbered_selections(input, &options, WIZARD_SPELLBOOK_SPELL_COUNT) {
                    Ok(names) => names,
        ...
        CreationStep::ChooseBackground => {
            ...
            let selected = input
                ...
                .or_else(|| {
                    all_bgs
                        .iter()
                        .copied()
                        .find(|bg| bg.to_string().to_lowercase() == input_lower)
                });
            ...
            let bg = match selected {
                ...
                None => {
                    return vec![format!(
                        "Please pick a background 1-{} or type its name.",
                        all_bgs.len()
                    )]
                }
            };
            ...
            let mut lines = vec![
                ...
                format!(
                    "(Background suggests: {} — press Enter or type 'default' to keep.)",
                    suggested
                ),
            ];
        ...
        CreationStep::ChooseOriginFeat => {
            ...
            state.game_phase =
                GamePhase::CharacterCreation(CreationStep::ChooseBackgroundAbilityPattern);
            let abilities = bg.ability_options();
            vec![
                format!(
                    "Origin feat: {}. Choose ability adjustment pattern:",
                    chosen
                ),
                format!(
                    "  1. +2 to {}, +1 to {} (ignores {})",
            ...
            match input {
                "1" | "standard" => {
                    ...
                    vec![
                        ...
                        "Assign scores to abilities. Enter six numbers (one per ability):"
                            .to_string(),
                ...
                _ => vec![
                    "Please choose 1 (Standard Array), 2 (Random), or 3 (Point Buy).".to_string(),
                ],
            }
        }
        CreationStep::PointBuy => {
            ...
            for &v in &values {
                match cost_table.get(&v) {
                    ...
                    None => {
                        return vec![format!(
                            "Invalid score: {}. Scores must be between 8 and 15.",
                            v
                        )]
                    }
                }
        ...
        CreationStep::AssignAbilities => {
            let available: Vec<i32> = state.log.iter().filter_map(|s| s.parse().ok()).collect();
            ...
            for &v in &values {
                if let Some(pos) = remaining.iter().position(|&x| x == v) {
                    ...
                    return vec![format!(
                        "Invalid assignment. Available scores: {:?}",
                        available
                    )];
                }
            }
        ...
        CreationStep::ChooseAlignment => {
                    ...
                    .find(|a| a.to_string().to_lowercase() == lower)
                {
                    ...
                    None => {
                        let names: Vec<String> =
                            ALIGNMENT_OPTIONS.iter().map(|a| a.to_string()).collect();
                        return vec![format!(
            ...
            vec![format!(
                "Alignment: {}. Enter your character's name:",
                chosen
            )]
        }
        CreationStep::ChooseName => {
            ...
            let mut lines = vec![
                format!(
                    ...
                    name, state.character.race, state.character.class
                ),
                String::new(),
            ];
...
fn parse_numbered_selections<'a>(
    ...
    for idx in indices {
        ...
        if !seen.insert(idx) {
            return Err(format!(
                "Duplicate choice: {}. Each spell must be different.",
                idx
            ));
        }
...
fn grant_background_equipment(state: &mut GameState) {
    ...
    let class_granted_names: std::collections::HashSet<String> = state
        .character
        .inventory
        .iter()
    ...
    loc.npcs
        ...
        .filter_map(|&id| {
            state
                ...
                .npcs
                .get(&id)
                .map(|npc| ((id as usize) | NPC_TAG, npc.name.clone()))
        })
        .collect()
}
...
fn room_feature_candidates(state: &GameState) -> Vec<(usize, String)> {
    let loc = match state.world.locations.get(&state.current_location) {
        Some(loc) => loc,
        None => return Vec::new(),
    };
    loc.room_features
        .iter()
        .enumerate()
        .map(|(index, feature)| ((index | ROOM_FEATURE_TAG) as usize, feature.name.clone()))
        .collect()
}

fn inspect_item(item: &state::Item) -> Vec<String> {
    use state::ItemType;

    let mut lines = vec![item.name.clone(), item.description.clone()];
    ...
    match &item.item_type {
        ItemType::Weapon {
            ...
            lines.push(format!(
                "Weapon: {}d{} {} damage.",
                damage_dice, damage_die, damage_type
            ));
            if *versatile_die > 0 {
                lines.push(format!(
                    "Versatile: 1d{} when used two-handed.",
                    versatile_die
                ));
            }
            if *range_normal > 0 {
                lines.push(format!("Range: {}/{} ft.", range_normal, range_long));
            }
        }
        ItemType::Armor {
            category,
            ...
            lines.push(format!("Armor: {:?}, AC {}.", category, base_ac));
            if *stealth_disadvantage {
                lines.push("Wearing it imposes disadvantage on Stealth checks.".to_string());
            }
        }
        ItemType::Consumable { effect } => {
            lines.push(format!("Consumable effect: {}.", effect));
        }
        ItemType::Key { unlocks } => {
            lines.push(format!("Key: it seems made to unlock {}.", unlocks));
        }
        ItemType::MagicWeapon {
            ...
            lines.push(format!(
                "Magic weapon: {}d{} {} damage, +{} attack, +{} damage.",
                damage_dice, damage_die, damage_type, attack_bonus, damage_bonus
            ));
            lines.push(format!("Rarity: {:?}.", rarity));
            if *requires_attunement {
                lines.push("Requires attunement.".to_string());
            }
        }
        ItemType::MagicArmor {
            category,
            ...
            lines.push(format!(
                "Magic armor: {:?}, AC {} (+{} bonus).",
                category, base_ac, ac_bonus
            ));
            lines.push(format!("Rarity: {:?}.", rarity));
            if *requires_attunement {
                lines.push("Requires attunement.".to_string());
        ...
        ItemType::Wondrous {
            ...
            lines.push(format!("Wondrous item. Rarity: {:?}.", rarity));
            if *requires_attunement {
                lines.push("Requires attunement.".to_string());
            }
        }
        ItemType::Potion { rarity, .. } => {
            lines.push(format!("Potion. Rarity: {:?}.", rarity));
        }
        ItemType::Scroll {
            spell_name,
            spell_level,
            ...
            lines.push(format!("Scroll: {} (level {}).", spell_name, spell_level));
            lines.push(format!("Rarity: {:?}.", rarity));
        }
        ItemType::Wand {
            spell_name,
            ...
            lines.push(format!("Wand: stores {}.", spell_name));
            if let Some(charges) = item.charges_remaining {
                lines.push(format!("Charges remaining: {}.", charges));
            }
            lines.push(format!("Rarity: {:?}.", rarity));
            if *requires_attunement {
                lines.push("Requires attunement.".to_string());
        ...
        ItemType::GearItem {
            ...
            lines.push(format!("Gear: {}.", gear_name));
            lines.push(format!(
                "Weight: {} lb. Cost: {} cp.",
                weight_qp / 4,
                cost_cp
...
fn search_target_candidates(state: &GameState) -> Vec<(usize, String)> {
    ...
    candidates.extend(room_feature_candidates(state));
    candidates
}
...
fn handle_look_target(state: &mut GameState, target: &str) -> Vec<String> {
    ...
    owned_candidates.extend(room_feature_candidates(state));
...
fn handle_search(state: &mut GameState, rng: &mut StdRng, target: Option<String>) -> Vec<String> {
    if let Some(target) = target {
        ...
        match resolver::resolve_target(&target, &candidates) {
            ...
            ResolveResult::Ambiguous(matches) => {
                return emit_disambiguation(state, "search", &matches);
            }
    ...
    let pending_trigger_ids: Vec<_> = loc
        .triggers
        .iter()
    ...
    let hidden_item_names: Vec<_> = loc
        .items
        .iter()
    ...
    if !live_npc_names.is_empty() {
        lines.push(format!(
            "You notice signs of activity from {}.",
            live_npc_names.join(", ")
        ));
    }
...
fn build_spell_targets(
    combat: &combat::CombatState,
    state: &GameState,
) -> Vec<spells::SpellTarget> {
    combat
        .initiative_order
        .iter()
        .filter_map(|(c, _)| {
    ...
    match command {
        ...
        Command::Go(direction) => {
            ...
            match next {
                                ...
                                let check_result = match &trigger.trigger_type {
                                    ...
                                    state::TriggerType::SavingThrow(ability) => {
                                        let score = state
                                            ...
                                            .unwrap_or(10);
                                        let is_prof =
                                            state.character.is_proficient_in_save(*ability);
                                        let disadv = armor_disadvantage_for_ability(
                                            &state.character,
                                            *ability,
                                        );
                ...
                None => {
                    vec![narration::templates::NO_EXIT
        ...
        Command::Talk(name) => {
            ...
            match resolver::resolve_target(&name, &candidates) {
                ...
                ResolveResult::Ambiguous(matches) => {
                    emit_disambiguation(state, "talk to", &matches)
                }
                ResolveResult::NotFound => {
                    vec![narration::templates::NPC_NOT_FOUND.replace("{name}", &name)]
                }
        ...
        Command::TakeAll => {
            ...
            let item_names: Vec<String> = item_ids
                .iter()
                .filter_map(|id| state.world.items.get(id).map(|item| item.name.clone()))
                .collect();
        ...
        Command::Inventory => {
            ...
            let mut grouped: std::collections::HashMap<String, (usize, String)> =
        ...
        Command::Check(skill_name) => {
            match parser::resolve_skill(&skill_name) {
                Some(skill) => {
                    ...
                    vec![narration::narrate_skill_check(
                        &mut rng,
                        &skill.to_string(),
                        &result,
                    )]
                ...
                None => vec![format!(
                    "Unknown skill: \"{}\". Try 'help' for a list.",
                    skill_name
                )],
            }
        }
        Command::Save(name) => {
            ...
            vec![format!(
                "[Game state ready to save as '{}.json'. Frontend handles file I/O.]",
                filename
            )]
        }
        Command::Load(name) => {
            let filename = name.unwrap_or_else(|| "autosave".to_string());
            vec![format!(
                "[Load '{}.json'. Frontend handles file I/O.]",
                filename
            )]
        }
        Command::Help(topic) => narration::templates::render_help(
            topic.as_deref(),
        ...
        Command::Cast {
            ...
            let spell_def = match spells::find_spell(&spell) {
                Some(def)
                    if state
                        ...
                        .known_spells
                        .iter()
                        .any(|s| s.to_lowercase() == def.name.to_lowercase()) =>
                {
                    def
                }
            ...
            if ritual {
                if !spell_def.ritual {
                    return vec![
                        narration::templates::CAST_NOT_A_RITUAL.replace("{spell}", spell_def.name)
                    ];
                }
                let mut lines = vec![
                    narration::templates::CAST_RITUAL_INTRO.replace("{spell}", spell_def.name)
                ];
                if spell_def.concentration {
                    match spells::begin_concentration(
                        ...
                        spells::ConcentrationStart::ReplacedPrior(prior) => {
                            lines.push(
                                narration::templates::CONCENTRATION_DROPPED
                                    .replace("{old}", &prior)
                        ...
                        spells::ConcentrationStart::Started => {
                            lines.push(
                                narration::templates::CONCENTRATION_STARTED
            ...
            match spell_def.name {
                ...
                "Cure Wounds" => {
                    ...
                    let mut out = Vec::new();
                    if let spells::CastOutcome::CureWoundsResult {
                        healing,
                        rolled,
                        modifier,
                    } = outcome
                    {
                        ...
                        if applied == 0 && state.character.current_hp == state.character.max_hp {
                            ...
                            out.push(
                                narration::templates::CAST_CURE_WOUNDS_SELF
                ...
                "Healing Word" => {
                    ...
                    let mut out = Vec::new();
                    if let spells::CastOutcome::HealingWordResult {
                        healing,
                        rolled,
                        modifier,
                    } = outcome
                    {
                        ...
                        if applied == 0 && state.character.current_hp == state.character.max_hp {
                            ...
                            out.push(
                                narration::templates::CAST_HEALING_WORD_SELF
                                    .replace("{roll}", &rolled.to_string())
                                    .replace("{mod}", &modifier.to_string())
                                    .replace("{healing}", &healing.to_string())
                                    .replace("{current}", &state.character.current_hp.to_string())
                ...
                "Mage Armor" => {
                    ...
                    vec![
                        ...
                        narration::templates::CAST_SLOT_USED
                            .replace(
                                "{remaining}",
                                &state.character.spell_slots_remaining[0].to_string(),
                            )
        ...
        Command::NewGame => {
            ...
            vec!["Start a new game? You will lose all current progress. (yes/no)".to_string()]
        }
        Command::ShortRest => rest::handle_short_rest(state, &mut rng),
        Command::LongRest => rest::handle_long_rest(state, &mut rng),
        ...
        Command::BardicInspiration(target) => {
            ...
            if state.character.class_features.bardic_inspiration_remaining == 0 {
                return vec![
                    "You have no Bardic Inspiration uses remaining. Rest to recover them."
                        .to_string(),
                ];
            }
            state.character.class_features.bardic_inspiration_remaining -= 1;
            let recipient = if target.is_empty() {
                "an ally".to_string()
            } else {
                target
            };
            vec![format!(
                "You inspire {}! They gain a Bardic Inspiration die.",
                recipient
            )]
        }
        Command::ChannelDivinity => {
            ...
            if state.character.class_features.channel_divinity_remaining == 0 {
                return vec![
                    "You have no Channel Divinity uses remaining. Rest to recover them."
                        .to_string(),
                ];
            }
        ...
        Command::LayOnHands(target) => {
            ...
            if state.character.class_features.lay_on_hands_pool == 0 {
                return vec![
                    "Your Lay on Hands pool is empty. Long rest to restore it.".to_string()
                ];
            }
            // Heal 5 HP per use from the pool (MVP: spend all available, cap at missing HP)
            let heal_amount = state
                .character
                .class_features
                .lay_on_hands_pool
            ...
            vec![format!(
                "You lay hands on {}, restoring {} HP. ({} HP remaining in pool)",
...
fn process_npc_turns(state: &mut GameState, rng: &mut StdRng) -> Vec<String> {
    ...
    loop {
        ...
        if let combat::Combatant::Npc(npc_id) = combatant {
            ...
            if let Some((old_dist, new_dist)) =
                should_trigger_opportunity_attack(&combat, state, npc_id)
            {
...
fn resolve_reaction_decision(state: &mut GameState, rng: &mut StdRng, accept: bool) -> Vec<String> {
    let mut lines = Vec::new();
    let mut combat = match state.active_combat.take() {
        Some(c) => c,
        None => return lines,
        ...
        combat::PendingReaction::OpportunityAttack {
            ...
            if accept {
                ...
                let target_ac = state
                    ...
                    .map(|s| s.ac)
                    .unwrap_or(10);
                let target_conditions: Vec<crate::conditions::ActiveCondition> = state
                    .world
                    .npcs
                    .get(&fleeing_npc_id)
                ...
                apply_sneak_attack(rng, state, &mut result, &mut lines, weapon_id, old_distance);
                if result.hit {
                    ...
                    lines.push(format!(
                        "You take an opportunity attack at {} -- miss.",
                        name
                    ));
                }
...
fn resolve_single_npc_attack(
    ...
    let npc_conditions: Vec<crate::conditions::ActiveCondition> = state
        .world
        .npcs
        .get(&npc_id)
        .map(|n| n.conditions.clone())
    ...
    let attack_ref = melee.or_else(|| {
        npc_attacks
            .iter()
            .find(|a| a.range_long > 0 && distance <= a.range_long as u32)
    });
    let attack = match attack_ref {
        Some(a) => a.clone(),
        None => return lines,
    };
    ...
    let grappled =
        crate::conditions::grappled_attack_disadvantage(&npc_conditions, &state.character.name);
...
fn check_player_concentration_on_damage(
    ...
    let save = spells::resolve_concentration_save(rng, con_score, con_prof, prof, damage_taken);
    if save.saved {
        lines.push(narration::templates::CONCENTRATION_HELD.replace("{spell}", &spell));
    } else {
        state.character.class_features.concentration_spell = None;
        lines.push(narration::templates::CONCENTRATION_BROKEN.replace("{spell}", &spell));
    }
}
    ...
    for i in 0..state.progress.objectives.len() {
        ...
        if let state::ObjectiveType::DefeatNpc(npc_id) = &state.progress.objective_triggers[i] {
            ...
            if npc_dead {
                ...
                lines.push(format!(
                    "Objective complete: {}",
                    state.progress.objectives[i].title
                ));
                newly_completed += 1;
            }
        }
...
fn append_player_turn_prompt(
    ...
    let hp_header = if let Some((enemy_name, enemy_hp, enemy_max)) = first_enemy {
        format!(
            "[HP: {}/{} | {}: {}/{}]",
            state.character.current_hp, state.character.max_hp, enemy_name, enemy_hp, enemy_max,
        )
    } else {
        format!(
            "[HP: {}/{}]",
...
fn handle_combat(state: &mut GameState, input: &str) -> Vec<String> {
    ...
    if has_pending {
        match command {
            ...
            _ => {
                ...
                let mut lines =
                    vec!["A reaction is pending -- respond with 'yes' or 'no'.".to_string()];
            ...
            match next_id {
                ...
                Some(next_id) => {
                    if !combat.player_disengaging {
                        let distance_changes: Vec<(types::NpcId, u32, u32)> = combat
                            .distances
                            .iter()
    ...
    match command {
            ...
            match resolver::resolve_target(&target_name, &candidates) {
                ResolveResult::Found(id) => {
                    ...
                    if let Some(weapon_item) = weapon_id.and_then(|id| state.world.items.get(&id)) {
                        if let state::ItemType::Weapon {
                            range_normal,
                            range_long,
                            properties,
                            ..
                        } = &weapon_item.item_type
                        {
                    ...
                    if result.hit {
                        if result.natural_20 {
                            if is_unarmed {
                                lines.push(format!(
                                    "You punch {} -- CRITICAL HIT! {} {} damage!",
                                    npc_name, result.damage, result.damage_type
                                ));
                            } else {
                                lines.push(format!(
                                    "You attack {} with {} -- CRITICAL HIT! {} {} damage!",
                        ...
                        if is_unarmed {
                            ...
                            lines.push(format!(
                                "You attack {} with {} -- natural 1, miss!",
                                npc_name, result.weapon_name
                            ));
                        }
                    } else if is_unarmed {
                        lines.push(format!(
                            "You swing at {} ({} vs AC {}) -- miss.",
                            npc_name,
                            format_roll(
                                result.attack_roll,
                                result.total_attack - result.attack_roll,
        ...
        Command::Approach(target_name) => {
            let owned_candidates: Vec<(usize, String)> = combat
                .initiative_order
                .iter()
                .filter_map(|(c, _)| {
                    if let combat::Combatant::Npc(id) = c {
            ...
            match resolver::resolve_target(&target_name, &candidates) {
                ResolveResult::Found(id) => {
                    let npc_id = id as u32;
                    let approach_lines =
                        combat::approach_target(&mut rng, npc_id, state, &mut combat);
        ...
        Command::Dash => {
            ...
            combat.action_used = true;
            lines.push(format!(
                "You take the Dash action. Movement this turn: {} ft.",
                combat.player_movement_remaining
            ));
        }
        Command::TakeCover => {
        ...
        Command::OffHandAttack(target_name) => {
            ...
            let (is_light_melee, is_weapon) = match state.world.items.get(&off_hand_id) {
                Some(item) => match &item.item_type {
                    state::ItemType::Weapon {
                        ...
                        let melee =
                            *range_normal == 0 || (properties & crate::equipment::THROWN != 0); // thrown-light is fine at 5ft
                        (light && melee, true)
                    }
                    _ => (false, false),
                },
            ...
            let main_hand_light_melee = match state
                .character
                .equipped
                .main_hand
                .and_then(|id| state.world.items.get(&id))
            {
                Some(item) => match &item.item_type {
                    state::ItemType::Weapon {
                        ...
                        let light = properties & crate::equipment::LIGHT != 0;
                        let melee =
                            *range_normal == 0 || (properties & crate::equipment::THROWN != 0);
                        light && melee
                    }
                    _ => false,
                },
            ...
            match resolver::resolve_target(&target_name, &candidates) {
                ResolveResult::Found(id) => {
                    ...
                    if nick_applies {
                        lines.push(
                            "Nick: this off-hand swing is part of your Attack action.".to_string(),
                        );
                    } else {
                        combat.bonus_action_used = true;
                    }
        ...
        Command::Use(item_name) => {
            ...
            if item_is_healing_potion(state, &item_name) {
                ...
                let (mut use_lines, consumed_action) =
                    resolve_use_item(state, &mut rng, &item_name);
                lines.append(&mut use_lines);
                if consumed_action {
                    combat.action_used = true;
                }
        ...
        Command::Cast {
            ...
            if spell_def.casting == spells::CastingMode::Reaction {
                state.active_combat = Some(combat);
                return vec![
                    narration::templates::CAST_REACTION_ONLY.replace("{spell}", spell_def.name)
                ];
            }
            ...
            match spell_def.name {
                            ...
                            if let spells::CastOutcome::FireBolt { attack, damage } = outcome {
                                if attack.hit {
                                    if attack.natural_20 {
                                        lines.push(
                                            narration::templates::CAST_FIRE_BOLT_CRIT
                                    ...
                                    lines.push(
                                        narration::templates::CAST_FIRE_BOLT_MISS_NAT1
                                            .replace("{target}", &npc_name),
                                    );
                                } else {
                                    lines.push(
                ...
                "Magic Missile" => {
                    ...
                    match resolver::resolve_target(&target_name, &candidates) {
                        ResolveResult::Found(id) => {
                            ...
                            if let spells::CastOutcome::MagicMissile {
                                darts,
                                total_damage,
                            } = outcome
                            {
                                lines.push(
                                    narration::templates::CAST_MAGIC_MISSILE
                                        .replace("{target}", &npc_name)
                ...
                "Burning Hands" => {
                    ...
                    let outcome =
                        spells::resolve_burning_hands(&mut rng, caster_score, prof_bonus, &targets);
                    if let spells::CastOutcome::BurningHands {
                        total_rolled,
                        half_damage: _,
                        dc,
                        results,
                    ...
                    {
                        lines.push(
                            narration::templates::CAST_BURNING_HANDS_INTRO
                        ...
                        for result in &results {
                            let save_str = format!(
                                ...
                                format_roll(
                                    ...
                                    result.save_result.modifier,
                                    result.save_result.total
                                ),
                                result.save_result.dc
                            );
                            if result.save_result.saved {
                                lines.push(
                                    narration::templates::CAST_BURNING_HANDS_SAVE
                                ...
                                lines.push(
                                    narration::templates::CAST_BURNING_HANDS_FAIL
                                        .replace("{target}", &result.name)
                                        .replace("{save_result}", &save_str)
                                        .replace("{damage}", &result.damage_taken.to_string()),
                                );
                ...
                "Sleep" => {
                    ...
                    if let spells::CastOutcome::SleepResult { hp_pool, affected } = outcome {
                        lines.push(
                            narration::templates::CAST_SLEEP_INTRO
                        ...
                        if affected.is_empty() {
                            ...
                            for target in &affected {
                                lines.push(
                                    narration::templates::CAST_SLEEP_TARGET
                ...
                "Sacred Flame" => {
                    ...
                    match resolver::resolve_target(&target_name, &candidates) {
                        ResolveResult::Found(id) => {
                            ...
                            let Some(spell_target) =
                                targets.iter().find(|t| t.id == npc_id).cloned()
                            else {
                                state.active_combat = Some(combat);
                                return vec![format!("There's no \"{}\" to target.", target_name)];
                            };
                            let npc_name = spell_target.name.clone();
                            let outcome = spells::resolve_sacred_flame(
                                &mut rng,
                                caster_score,
                                prof_bonus,
                                &spell_target,
                            );
                            if let spells::CastOutcome::SacredFlame {
                                save_result,
                                damage,
                            } = outcome
                            {
                                ...
                                if save_result.saved {
                                    lines.push(
                                        narration::templates::CAST_SACRED_FLAME_SAVE
                                            .replace("{target}", &npc_name)
                                            .replace("{save_result}", &save_str),
                                    );
                                } else {
                                    lines.push(
                                        narration::templates::CAST_SACRED_FLAME_HIT
                                            .replace("{target}", &npc_name)
                                            .replace("{save_result}", &save_str)
                                            .replace("{damage}", &damage.to_string()),
                                    );
                        ...
                        ResolveResult::Found(id) => {
                            ...
                            if let spells::CastOutcome::GuidingBolt { attack, damage } = outcome {
                                if attack.hit {
                                    if attack.natural_20 {
                                        lines.push(
                                            narration::templates::CAST_GUIDING_BOLT_CRIT
                                                .replace("{target}", &npc_name)
                                                .replace("{damage}", &damage.to_string()),
                                        );
                                    } else {
                                    ...
                                    lines.push(
                                        narration::templates::CAST_GUIDING_BOLT_MISS_NAT1
                                            .replace("{target}", &npc_name),
                                    );
                                } else {
                                    lines.push(
                ...
                "Vicious Mockery" => {
                    ...
                    match resolver::resolve_target(&target_name, &candidates) {
                        ResolveResult::Found(id) => {
                            ...
                            let npc_name = spell_target.name.clone();
                            let outcome = spells::resolve_vicious_mockery(
                                &mut rng,
                                caster_score,
                                prof_bonus,
                                &spell_target,
                            );
                            if let spells::CastOutcome::ViciousMockery {
                                save_result,
                                damage,
                            } = outcome
                            {
                                ...
                                if save_result.saved {
                                    lines.push(
                                        narration::templates::CAST_VICIOUS_MOCKERY_SAVE
                                            .replace("{target}", &npc_name)
                                            .replace("{save_result}", &save_str),
                                    );
                                } else {
                                    lines.push(
                                        narration::templates::CAST_VICIOUS_MOCKERY_HIT
                                            .replace("{target}", &npc_name)
                                            .replace("{save_result}", &save_str)
                                            .replace("{damage}", &damage.to_string()),
                                    );
                ...
                "Charm Person" => {
                    ...
                    match resolver::resolve_target(&target_name, &candidates) {
                        ResolveResult::Found(id) => {
                            ...
                            let npc_name = spell_target.name.clone();
                            let outcome = spells::resolve_charm_person(
                                &mut rng,
                                caster_score,
                                prof_bonus,
                                &spell_target,
                            ...
                            if let spells::CastOutcome::CharmPerson { save_result } = outcome {
                                ...
                                if save_result.saved {
                                    lines.push(
                                        narration::templates::CAST_CHARM_PERSON_SAVE
                                            .replace("{target}", &npc_name)
                                            .replace("{save_result}", &save_str),
                                    );
                                } else {
                                    lines.push(
                                        narration::templates::CAST_CHARM_PERSON_HIT
                                            .replace("{target}", &npc_name)
                                            .replace("{save_result}", &save_str),
                                    );
                                }
                ...
                "Faerie Fire" => {
                    ...
                    match resolver::resolve_target(&target_name, &candidates) {
                        ResolveResult::Found(id) => {
                            ...
                            let npc_name = spell_target.name.clone();
                            let outcome = spells::resolve_faerie_fire(
                                &mut rng,
                                caster_score,
                                prof_bonus,
                                &spell_target,
                            ...
                            if let spells::CastOutcome::FaerieFire { save_result } = outcome {
                                ...
                                if save_result.saved {
                                    lines.push(
                                        narration::templates::CAST_FAERIE_FIRE_SAVE
                                            .replace("{target}", &npc_name)
                                            .replace("{save_result}", &save_str),
                                    );
                                } else {
                                    lines.push(
                                        narration::templates::CAST_FAERIE_FIRE_HIT
                                            .replace("{target}", &npc_name)
                                            .replace("{save_result}", &save_str),
                                    );
                                }
                        ...
                        ResolveResult::Found(id) => {
                            ...
                            if let spells::CastOutcome::EldritchBlast { attack, damage } = outcome {
                                if attack.hit {
                                    if attack.natural_20 {
                                        lines.push(
                                            narration::templates::CAST_ELDRITCH_BLAST_CRIT
                                                .replace("{target}", &npc_name)
                                                .replace("{damage}", &damage.to_string()),
                                        );
                                    } else {
                                    ...
                                    lines.push(
                                        narration::templates::CAST_ELDRITCH_BLAST_MISS_NAT1
                                            .replace("{target}", &npc_name),
                                    );
                                } else {
                                    lines.push(
        ...
        Command::Grapple(target_name) => {
            ...
            if distance > 5 {
                state.active_combat = Some(combat);
                return vec![
                    "You're too far away to grapple. Move closer first (approach <target>)."
                        .to_string(),
                ];
            }
                ...
                Some(r) => {
                    ...
                    if r.success {
                        lines.push(format!(
                            ...
                            format_roll(
                                r.target_d20,
                                r.target_save_total - r.target_d20,
                                r.target_save_total
                            ),
                            r.dc,
                            npc_name
        ...
        Command::Shove(ref target_name) | Command::ShoveProne(ref target_name) => {
            ...
            if distance > 5 {
                state.active_combat = Some(combat);
                return vec![
                    "You're too far away to shove. Move closer first (approach <target>)."
                        .to_string(),
                ];
            }
            // Size check: target must not be more than 1 size category larger.
            ...
            if combat::target_exceeds_grapple_size_limit(&shover_size, &target_size) {
                state.active_combat = Some(combat);
                return vec!["The target is too large to shove.".to_string()];
            }
            combat.action_used = true;
    ...
    match resolver::resolve_target(&item_name, &candidates) {
        ResolveResult::Found(id) => {
            ...
            match item_type {
                ...
                Some(state::ItemType::Wand {
                    ref spell_name,
                    requires_attunement,
                    ..
                }) => {
                    let result =
                        use_magic_wand(state, &name, spell_name, requires_attunement, item_id);
                ...
                Some(state::ItemType::Scroll {
                    ref spell_name,
                    spell_level,
                    ..
                }) => {
                    let (lines, consumed) =
                        use_magic_scroll(state, rng, &name, spell_name, spell_level);
                    if consumed {
                        state.character.inventory.retain(|&id| id != item_id);
                        state.world.items.remove(&item_id);
                    }
                    ...
                    let result = match effect.as_str() {
                        ...
                        "light" => {
                            ...
                            if let Some(loc) = state.world.locations.get_mut(&loc_id) {
                                ...
                                vec![narration::templates::USE_UNKNOWN_EFFECT
                                    .replace("{item}", &name)]
                            }
                        }
                        "nourish" => {
...
fn use_magic_potion(
    ...
    match effect {
        PotionEffect::Healing { dice, die, bonus } => {
            ...
            let old_hp = state.character.current_hp;
            state.character.current_hp =
                (state.character.current_hp + roll_total).min(state.character.max_hp);
            let healed = state.character.current_hp - old_hp;
            if healed > 0 {
                vec![narration::templates::USE_HEAL
...
fn use_magic_wand(
    ...
    let charges = state
        ...
        .items
        .get(&item_id)
        .and_then(|i| i.charges_remaining)
        .unwrap_or(0);
    if charges == 0 {
        return vec![format!(
...
fn seed_objectives(state: &mut GameState) {
    use state::{Disposition, Objective, ObjectiveType};

    // Find the first hostile NPC with combat stats to designate as boss
    let mut hostile_npcs: Vec<_> = state
        .world
        .npcs
        .values()
    ...
    if let Some(boss) = hostile_npcs.first() {
        ...
        state.progress.objectives.push(Objective {
            ...
            description: format!(
                "A dangerous foe known as {} lurks in {}. Defeat them to prove your worth.",
                boss_name, location_name
            ),
            completed: false,
        });
        state
            .progress
            .objective_triggers
            .push(ObjectiveType::DefeatNpc(boss_id));
    }
}
...
fn render_map(state: &GameState) -> Vec<String> {
    ...
    if ids.is_empty() {
        return vec![
            "=== MAP ===".to_string(),
            "No locations discovered yet.".to_string(),
        ];
    }
...
fn apply_feat_effects(character: &mut character::Character, feat_name: &str) {
    let Some(feat) = FeatDef::lookup(feat_name) else {
        return;
    };
    for effect in feat.effects {
        match effect {
            ...
            FeatEffect::LanguageProficiency | FeatEffect::ToolProficiency | FeatEffect::Flavor => {}
        }
    }
}
...
fn handle_choose_asi(state: &mut GameState, input: &str) -> Vec<String> {
    ...
    if matches!(lower.as_str(), "cancel" | "later" | "skip") {
        state.game_phase = GamePhase::Exploration;
        return vec![
            "You set the decision aside for now. Type 'asi' or rest to revisit.".to_string(),
        ];
    }
        ...
        .or_else(|| trimmed.strip_prefix("+2"))
    {
        ...
        let Some(ab) = parse_ability_code(parts[0]) else {
            return vec![format!(
                "Unknown ability '{}'. Use STR/DEX/CON/INT/WIS/CHA.",
                parts[0]
            )];
        };
    ...
    if let Some(feat) = FeatDef::lookup(trimmed) {
        ...
        {
            return vec![
                "Only Fighters, Paladins, and Rangers can take fighting-style feats.".to_string(),
            ];
        }
...
fn finalize_asi(state: &mut GameState, mut lines: Vec<String>) -> Vec<String> {
    if state.character.asi_credits == 0 {
        ...
        lines.push(format!(
            ...
            state.character.asi_credits,
            if state.character.asi_credits == 1 {
                ""
            } else {
                "s"
            },
...
fn asi_menu_lines(state: &GameState) -> Vec<String> {
    let mut lines = vec![
        ...
        "  - feat name (e.g. 'Tough', 'Defense') -> take a general or fighting-style feat"
            .to_string(),
...
fn armor_disadvantage_for_ability(
    ...
    matches!(
        ability,
        types::Ability::Strength | types::Ability::Dexterity
    )
}
...
fn handle_equip_command(state: &mut GameState, target_str: &str) -> Vec<String> {
    ...
    let (target_name, force_off_hand) = if words.len() >= 3
        && words[words.len() - 2] == "off"
        && words[words.len() - 1] == "hand"
    {
    ...
    match resolver::resolve_target(&target_name, &candidates) {
        ResolveResult::Found(id) => {
            ...
            let item = match state.world.items.get(&item_id) {
                ...
                None => {
                    return vec![
                        narration::templates::EQUIP_NOT_FOUND.replace("{name}", &target_name)
                    ]
                }
            };
            ...
            match &item.item_type {
                state::ItemType::Weapon { properties, .. } => {
                    ...
                    if force_off_hand && !is_light {
                        return vec![format!(
                            "The {} is too unwieldy to wield in your off hand.",
                            item.name
                        )];
                    }
                    ...
                    if is_two_handed {
                        if let Some(old_id) = state.character.equipped.main_hand.take() {
                            if old_id != item_id {
                                let old_name = state
                                    ...
                                    .unwrap_or_default();
                                result_lines.push(
                                    narration::templates::EQUIP_SWAP_WEAPON
                        ...
                        if let Some(oh_id) = state.character.equipped.off_hand.take() {
                            let oh_name = state
                                ...
                                .unwrap_or_default();
                            result_lines.push(
                                narration::templates::EQUIP_TWO_HAND_CLEAR
                        ...
                        if result_lines.is_empty() {
                            result_lines.push(
                                narration::templates::EQUIP_WIELD.replace("{item}", &item.name),
                            );
                        }
                        ...
                        if result_lines.is_empty() {
                            result_lines.push(
                                narration::templates::EQUIP_WIELD_OFF.replace("{item}", &item.name),
                            );
                        }
                    } else {
                        if state.character.equipped.off_hand == Some(item_id) {
                ...
                state::ItemType::Armor { category, .. } => {
                    ...
                    if *category == state::ArmorCategory::Shield {
                        ...
                        if result_lines.is_empty() {
                            result_lines.push(
                                narration::templates::EQUIP_SHIELD.replace("{item}", &item.name),
                            );
                        }
                    } else {
                        if let Some(old_id) = state.character.equipped.body.replace(item_id) {
                            if old_id != item_id {
                                let old_name = state
                                    ...
                                    .unwrap_or_default();
                                result_lines.push(
                                    narration::templates::EQUIP_SWAP_ARMOR
                                        .replace("{old}", &old_name)
                                        .replace("{new}", &item.name),
                                );
                            }
                        ...
                        if result_lines.is_empty() {
                            result_lines.push(
                                narration::templates::EQUIP_WEAR.replace("{item}", &item.name),
                            );
                        }
        ...
        ResolveResult::NotFound => {
            vec![narration::templates::EQUIP_NOT_FOUND.replace("{name}", &target_name)]
        }
    }
}
...
fn handle_unequip_command(state: &mut GameState, target_str: &str) -> Vec<String> {
    ...
    let candidates: Vec<(usize, &str)> = equipped_candidates
        .iter()
        .map(|(id, name)| (*id, name.as_str()))
        .collect();
    ...
    match resolver::resolve_target(target_str, &candidates) {
        ...
        ResolveResult::NotFound => {
            vec![narration::templates::UNEQUIP_NOT_EQUIPPED.replace("{name}", target_str)]
        }
    }
}
...
fn magic_weapon_bonuses(state: &GameState, weapon_id: Option<crate::types::ItemId>) -> (i32, i32) {
...
fn apply_mainhand_mastery_effects(
    ...
    match mastery {
        ...
        Mastery::Cleave => {
            if let Some((secondary_id, cleave_result, _mod)) =
                combat::apply_cleave_mastery(rng, true, result, npc_id, combat, state, ability_mod)
            {
                ...
                if cleave_result.hit {
                    ...
                    if let Some(npc) = state.world.npcs.get_mut(&secondary_id) {
                        let _dealt = combat::apply_damage_to_npc(
                            npc,
                            cleave_result.damage,
                            cleave_result.damage_type,
                            lines,
                        );
                        if let Some(stats) = npc.combat_stats.as_ref() {
                            if stats.current_hp <= 0 {
                    ...
                    lines.push(format!(
                        "Cleave: your follow-through misses {}.",
                        secondary_name,
                    ));
                }
            }
    ...
    match resolver::resolve_target(target, &candidates) {
        ResolveResult::Found(id) => {
            ...
            if !item_is_magical(&item) {
                return vec![format!(
                    "The {} is not a magic item. Only magic items can be attuned.",
                    item.name
                )];
            }
            ...
            vec![format!(
                "You attune to the {}. You feel its power resonate with you.",
                item.name
            )]
        }
...
fn handle_unattune_command(state: &mut GameState, target_str: &str) -> Vec<String> {
    ...
    let attuned_candidates: Vec<(usize, String)> = state
        .character
        .attuned_items
        .iter()
        .filter_map(|id| {
            state
                ...
                .items
                .get(id)
                .map(|i| (*id as usize, i.name.clone()))
        })
        .collect();
    let candidates: Vec<(usize, &str)> = attuned_candidates
        .iter()
        .map(|(id, name)| (*id, name.as_str()))
        .collect();
...
fn handle_use_tool_command(state: &mut GameState, tool_name: &str, target: &str) -> Vec<String> {
    ...
    let check_ability = maybe_tool_prof
        .as_ref()
    ...
    let prof_bonus = if is_proficient {
        state.character.proficiency_bonus()
    } else {
        0
    };
    ...
    lines.push(format!("You use your {} on the {}.", tool_name, target));
    lines.push(format!(
        "{} check{}: rolled {} + {} = {}",
    ...
    if total >= 15 {
        lines.push(format!(
            "Success! You expertly apply the {} to the {}.",
            tool_name, target
        ));
    } else if total >= 10 {
        lines.push(format!(
            "Partial success. You manage to use the {} on the {}, though not perfectly.",
            tool_name, target
        ));
    } else {
        lines.push(format!(
            "You struggle with the {} and fail to affect the {}.",
            tool_name, target
        ));
    }
    lines
...
mod tests {
    ...
    fn test_choose_class_rejects_invalid_input() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|l| l.to_lowercase().contains("class") || l.contains("1")),
            "Got: {:?}",
            output.text
        );
    }
    ...
    fn test_elf_goes_to_choose_subrace() {
        ...
        assert!(matches!(
            state.game_phase,
            GamePhase::CharacterCreation(CreationStep::ChooseSubrace)
        ));
    ...
    fn test_drow_finalized_has_darkvision_120() {
        ...
        assert!(state
            .character
            .traits
            .contains(&"Darkvision 120 ft".to_string()));
    ...
    fn test_high_elf_finalized_has_prestidigitation() {
        ...
        assert!(state
            .character
            .traits
            .contains(&"Prestidigitation cantrip".to_string()));
    }
    ...
    fn test_dragonborn_red_finalized_has_fire_traits() {
        ...
        assert!(state
            .character
            .traits
            .contains(&"Fire Breath Weapon".to_string()));
        assert!(state
            .character
            .traits
            .contains(&"Fire Resistance".to_string()));
    }
    ...
    fn test_orc_finalized_has_correct_traits() {
        ...
        assert!(state
            .character
            .traits
            .contains(&"Adrenaline Rush".to_string()));
        assert!(state
            .character
            .traits
            .contains(&"Relentless Endurance".to_string()));
    }
    ...
    fn test_goliath_finalized_has_35_speed() {
        ...
        assert!(state
            .character
            .traits
            .contains(&"Powerful Build".to_string()));
        assert!(state
            .character
            .traits
            .contains(&"Cloud's Jaunt".to_string()));
    }
    ...
    fn test_tiefling_infernal_finalized_has_fire_resistance() {
        ...
        assert!(state
            .character
            .traits
            .contains(&"Fire Bolt cantrip".to_string()));
    }
    ...
    fn test_gnome_forest_finalized_has_minor_illusion() {
        ...
        assert!(state
            .character
            .traits
            .contains(&"Minor Illusion cantrip".to_string()));
    }
    ...
    fn test_pending_subrace_cleared_after_finalization() {
        ...
        assert_eq!(
            state.pending_subrace, None,
            "pending_subrace should be cleared after finalization"
        );
    }
    ...
    fn test_new_species_have_empty_ability_bonuses_in_creation() {
        ...
        assert_eq!(
            state.character.ability_scores[&Ability::Strength],
            15,
    ...
    fn test_rage_dispatch_non_barbarian_errors_and_no_state_change() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|l| l.to_lowercase().contains("barbarian")),
            "Expected 'barbarian' in error. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_rage_dispatch_barbarian_zero_uses_errors_and_no_state_change() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|l| l.to_lowercase().contains("no rage")),
            "Expected 'no Rage' in error. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_bardic_inspiration_dispatch_non_bard_errors() {
        ...
        assert!(output
            .text
            .iter()
            .any(|l| l.to_lowercase().contains("bard")));
    }
    ...
    fn test_bardic_inspiration_dispatch_bard_zero_uses_errors() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|l| l.to_lowercase().contains("no bardic inspiration")),
            "Got: {:?}",
            output.text
        );
    }
    ...
    fn test_channel_divinity_dispatch_non_cleric_paladin_errors() {
        ...
        assert!(output
            .text
            .iter()
            .any(|l| l.to_lowercase().contains("cleric") || l.to_lowercase().contains("paladin")));
    }
    ...
    fn test_channel_divinity_dispatch_level_one_cleric_errors() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|l| l.to_lowercase().contains("no channel")),
            "Got: {:?}",
            output.text
        );
    }
    ...
    fn test_channel_divinity_dispatch_cleric_with_uses_decrements() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|l| l.to_lowercase().contains("divine")),
            "Got: {:?}",
            output.text
        );
    }
    ...
    fn test_lay_on_hands_dispatch_non_paladin_errors() {
        ...
        assert!(output
            .text
            .iter()
            .any(|l| l.to_lowercase().contains("paladin")));
    }
    ...
    fn test_lay_on_hands_dispatch_paladin_heals_and_deducts_pool() {
        ...
        assert!(output
            .text
            .iter()
            .any(|l| l.to_lowercase().contains("lay hands")
    ...
    fn test_lay_on_hands_dispatch_empty_pool_errors() {
        ...
        assert!(output
            .text
            .iter()
            .any(|l| l.to_lowercase().contains("empty") || l.to_lowercase().contains("no lay")));
    }
    ...
    fn test_ki_dispatch_non_monk_errors() {
        ...
        assert!(output
            .text
            .iter()
            .any(|l| l.to_lowercase().contains("monk")));
    }
    ...
    fn test_choose_alignment_prompt_lists_all_ten_options() {
        ...
        let joined = output.text.join("\n");
        assert!(
            joined.contains("Lawful Good"),
            "prompt should list Lawful Good. Got:\n{}",
            joined
        );
        assert!(
            joined.contains("Chaotic Evil"),
            "prompt should list Chaotic Evil. Got:\n{}",
            joined
        );
        assert!(
            joined.contains("Unaligned"),
            "prompt should list Unaligned. Got:\n{}",
            joined
        );
        assert!(
            joined.contains("alignment"),
            "prompt should mention alignment. Got:\n{}",
            joined
        );
    }
    ...
    fn test_choose_alignment_numeric_selection_sets_alignment_and_advances() {
        ...
        assert!(
            ...
            "Expected name prompt after alignment. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_choose_alignment_accepts_all_ten_by_number() {
        ...
        for (idx, expected_alignment) in expected.iter().enumerate() {
            ...
            assert_eq!(
                ...
                "input {} should set alignment {:?}",
                input, expected_alignment,
            );
        }
    }
    ...
    fn test_full_character_creation_flow() {
        ...
        let output = process_input(&output.state_json, "Fighter");
        assert!(
            output.text.iter().any(|t| t.contains("background")),
            "Expected background prompt after class. Got: {:?}",
            output.text
        );
        ...
        let output = process_input(&output.state_json, "1");
        assert!(
            output.text.iter().any(|t| t.contains("origin feat")),
            "Expected origin-feat prompt. Got: {:?}",
            output.text
        );
        ...
        let output = process_input(&output.state_json, "default");
        assert!(
            output.text.iter().any(|t| t.contains("adjustment pattern")),
            "Expected ability pattern prompt. Got: {:?}",
            output.text
        );
        ...
        let output = process_input(&output.state_json, "1 2");
        assert!(
            output.text.iter().any(|t| t.contains("alignment")),
            "Expected alignment prompt after skills. Got: {:?}",
            output.text
        );
        ...
        assert!(state
            ...
            .traits
            .iter()
            .any(|t| t == "Origin Feat: Magic Initiate (Cleric)"));
    }
    ...
    fn test_background_grants_skill_and_tool_proficiencies() {
        ...
        assert!(state
            .character
            .languages
            .contains(&"Thieves' Cant".to_string()));
        // Origin feat trait
        assert!(state
            .character
            .traits
            .iter()
            .any(|t| t == "Origin Feat: Alert"));
    }
    ...
    fn test_background_ability_adjustment_plus_two_plus_one() {
        ...
        assert_eq!(
            state.character.ability_scores[&Ability::Dexterity],
            17,
            "DEX 14 + Human 1 + Criminal 2 = 17"
        );
        assert_eq!(
            state.character.ability_scores[&Ability::Constitution],
            15,
            "CON 13 + Human 1 + Criminal 1 = 15"
        );
    ...
    fn test_background_invalid_input_reprompts() {
        ...
        assert!(output
            .text
            .iter()
            .any(|t| t.contains("1-16") || t.contains("background")));
    }
    ...
    fn test_fighter_gets_starting_equipment() {
        ...
        assert!(
            state.character.equipped.main_hand.is_some(),
            "main_hand should be equipped"
        );
        assert!(
            state.character.equipped.off_hand.is_some(),
            "off_hand should be equipped"
        );
        assert!(
            state.character.equipped.body.is_some(),
            "body should be equipped"
        );
    ...
    fn test_wizard_gets_starting_equipment() {
        ...
        assert!(
            state.character.equipped.body.is_none(),
    ...
    fn test_wizard_sage_starts_with_one_quarterstaff() {
        ...
        assert_eq!(
            quarterstaff_count, 1,
            "Wizard + Sage should start with one Quarterstaff"
        );
    }
    ...
    fn test_starting_equipment_ids_dont_collide_with_world_items() {
        ...
        for &inv_id in &state.character.inventory {
            assert!(
                state.world.items.contains_key(&inv_id),
                "Inventory item {} should exist in world items",
                inv_id
            );
        }
    }
    ...
    fn test_barbarian_starting_ac_uses_unarmored_defense() {
        ...
        assert_eq!(
            ac, 14,
            "Barbarian AC should use Unarmored Defense (10 + DEX + CON)"
        );
    }
    ...
    fn test_inventory_empty() {
        ...
        assert!(output
            .text
            .iter()
            .any(|t| t.contains("carrying") || t.contains("anything")));
    }
    ...
    fn test_look_at_inventory_item() {
        ...
        assert!(
            ...
            "Should find item '{}' in inventory. Got: {:?}",
            item_name,
            output.text
        );
        assert!(
            output.text.len() >= 2,
            "Expected richer item inspection output. Got: {:?}",
            output.text
        );
    }
            ...
            state::Npc {
                ...
                combat_stats: Some(state::CombatStats {
                    special_traits: vec![(
                        ...
                        "Gains advantage when ally is adjacent.".to_string(),
                    )],
    ...
    #[test]
    fn test_look_at_room_feature_returns_feature_description() {
        let mut state = create_test_exploration_state();
        let loc = state
            .world
            .locations
        ...
        let state_json = serde_json::to_string(&state).unwrap();
        let output = process_input(&state_json, "examine altar");

        assert_eq!(output.text[0], "altar");
        assert!(output
            .text
            .iter()
            .any(|line| line.contains("soot-dark prayers")));
    }

    #[test]
    fn test_look_at_room_feature_uses_disambiguation() {
        let mut state = create_test_exploration_state();
        let loc = state
            .world
            .locations
        ...
        let state_json = serde_json::to_string(&state).unwrap();
        let output = process_input(&state_json, "look door");
    ...
    fn test_drop_command() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("drop")),
            "Should confirm drop. Got: {:?}",
            output.text
        );
    ...
    fn test_take_all_picks_up_every_room_item() {
        ...
        let new_state: GameState = serde_json::from_str(&output.state_json).unwrap();
        assert!(
            new_state.world.locations[&new_state.current_location]
                .items
                .is_empty(),
    ...
    fn test_fuzzy_target_npc() {
        ...
        if let Some(&npc_id) = loc.npcs.first() {
            ...
            let output = process_input(
                &state_json,
                &format!("talk {}", first_3_chars.to_lowercase()),
            );
            let all_text = output.text.join(" ");
            assert!(
                !all_text.contains("no one called"),
                "Fuzzy match should find NPC with prefix '{}'. Got: {:?}",
                first_3_chars,
                output.text
            );
        }
    ...
    fn test_talk_to_verb_phrase() {
        ...
        if let Some(&npc_id) = loc.npcs.first() {
            ...
            assert!(
                output.text.iter().any(|t| t.contains("says:")),
                "Should talk to NPC. Got: {:?}",
                output.text
            );
        }
    }
    ...
    fn test_equip_weapon_from_inventory() {
        ...
        let output = process_input(
            &state_json,
            &format!("equip {}", weapon_name.to_lowercase()),
        );
        let new_state: GameState = serde_json::from_str(&output.state_json).unwrap();
        assert!(
            new_state.character.equipped.main_hand == Some(weapon_id),
            "Weapon should be in main hand. Got: {:?}",
            new_state.character.equipped
        );
    }
    ...
    fn test_equip_armor_from_inventory() {
        ...
        state.world.items.insert(
            ...
            state::Item {
                ...
                description: "Leather armor.".to_string(),
                item_type: state::ItemType::Armor {
    ...
    fn test_equip_auto_swap() {
        ...
        state.world.items.insert(
            ...
            state::Item {
                ...
                description: "A long sword.".to_string(),
                item_type: state::ItemType::Weapon {
                    damage_dice: 1,
                    damage_die: 8,
        ...
        let new_state: GameState = serde_json::from_str(&output.state_json).unwrap();
        assert_eq!(
            new_state.character.equipped.main_hand,
            Some(id2),
            "Should swap to longsword"
        );
    ...
    fn test_unequip_weapon() {
        ...
        let output = process_input(
            &state_json,
            &format!("unequip {}", weapon_name.to_lowercase()),
        );
        let new_state: GameState = serde_json::from_str(&output.state_json).unwrap();
    ...
    fn test_character_sheet_shows_ac() {
        ...
        assert!(
            output.text.iter().any(|t| t.contains("AC:")),
            "Character sheet should show AC. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_character_sheet_barbarian_shows_unarmored_defense_ac() {
        ...
        state
            .character
            .ability_scores
            .insert(Ability::Dexterity, 14);
        state
            .character
            .ability_scores
            .insert(Ability::Constitution, 16);

        let state_json = serde_json::to_string(&state).unwrap();
        let output = process_input(&state_json, "character");
        assert!(
            output.text.iter().any(|t| t == "AC: 15"),
            "Character sheet should reflect Barbarian Unarmored Defense. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_combat_look_barbarian_shows_unarmored_defense_ac() {
        ...
        assert!(
            output.text.iter().any(|t| t == "AC: 15"),
            "Combat look should reflect Barbarian Unarmored Defense. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_inventory_shows_equipped() {
        ...
        let output = process_input(&state_json, "inventory");
        assert!(
            output.text.iter().any(|t| t.contains("equipped")),
            "Inventory should mark equipped items. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_combat_blocks_exploration_commands() {
        ...
        assert!(
            ...
            "Go during combat should attempt to flee. Got: {:?}",
            go_output.text
        );
        ...
        assert!(
            take_output
                .text
                .iter()
                .any(|t| t.contains("can't do that during combat")),
            "Take should be blocked. Got: {:?}",
            take_output.text
        );
    }
    ...
    fn test_combat_allows_look() {
        ...
        let output = process_input(&state_json, "look");
        assert!(
            output.text.iter().any(|t| t.contains("Combat")),
            "Look should show combat status. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_combat_allows_inventory() {
        ...
        let output = process_input(&state_json, "inventory");
        assert!(
            output.text.iter().any(|t| t.contains("carrying")),
            "Inventory should work in combat. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_combat_allows_help() {
        ...
        let output = process_input(&state_json, "help");
        assert!(
            output.text.iter().any(|t| t.contains("attack")),
            "Help should show combat commands. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_combat_dodge_action() {
        ...
        assert!(
            output.text.iter().any(|t| t.contains("Dodge")),
            "Should confirm dodge. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_combat_disengage_action() {
        ...
        assert!(
            output.text.iter().any(|t| t.contains("Disengage")),
            "Should confirm disengage. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_combat_dash_action() {
        ...
        assert!(
            output.text.iter().any(|t| t.contains("Dash")),
            "Should confirm dash. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_combat_attack_keeps_turn_open_when_movement_remaining() {
        ...
        assert!(
            ...
            "Attack should not auto-end turn when movement remains"
        );
    ...
    fn test_combat_end_turn_advances_to_npc_and_back() {
        ...
        assert!(
            output.text.iter().any(|t| t.contains("Your turn!")),
            "Expected player turn prompt after NPC turns, got: {:?}",
            output.text
        );
    }
    ...
    fn test_attack_with_zero_movement_keeps_turn_open_until_end_turn() {
        ...
        assert!(
            output.text.iter().any(|t| t.contains("Your turn!")),
            "Expected player turn prompt to continue, got: {:?}",
            output.text
        );
    }
    ...
    fn test_attack_then_bonus_dash_allowed_on_same_turn() {
        ...
        let after_attack: GameState = serde_json::from_str(&output.state_json).unwrap();
        {
            ...
            assert!(
                combat.is_player_turn(),
                "Turn should remain with player after attack"
            );
    ...
    fn test_ranged_attack_at_distance_5_with_other_hostile_in_melee_has_disadvantage() {
        ...
        if let Some(ref mut combat) = state.active_combat {
            ...
            combat
                .initiative_order
                .push((combat::Combatant::Npc(second_npc_id), 5));
        }
    ...
    fn test_combat_not_in_combat_message() {
        ...
        let output = process_input(&state_json, "attack goblin");
        assert!(
            output.text.iter().any(|t| t.contains("not in combat")),
            "Should say not in combat. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_combat_use_healing_potion_heals_and_consumes_action() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.contains("HP") || t.contains("recover")),
            "{:?}",
            output.text
        );
        ...
        assert!(
            ...
            "Using potion should consume bonus action in combat (2024 SRD)"
        );
        assert!(
            !combat.action_used,
            "Using potion should NOT consume the main action"
        );
    ...
    fn test_turn_status_shows_all_four_resources() {
        ...
        assert!(
            ...
            "Expected action status, got: {:?}",
            output.text
        );
        assert!(
            joined.contains("bonus"),
            "Expected bonus status, got: {:?}",
            output.text
        );
        assert!(
            joined.contains("reaction"),
            "Expected reaction status, got: {:?}",
            output.text
        );
        assert!(
            joined.contains("movement"),
            "Expected movement status, got: {:?}",
            output.text
        );
    }
    ...
    fn test_combat_help_mentions_bonus_and_reaction() {
        ...
        assert!(
            joined.contains("bonus"),
            "Combat help should mention bonus actions, got: {:?}",
            output.text
        );
        assert!(
    ...
    fn test_shield_reaction_prompt_fires_when_npc_hits_wizard() {
        ...
        assert!(
            ...
            "Shield reaction prompt should be set when NPC hit would land. Got text: {:?}",
            output.text
        );
        assert!(
            matches!(
                combat.pending_reaction,
                Some(combat::PendingReaction::Shield { .. })
            ),
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("shield")
                    && (t.to_lowercase().contains("yes")
                        || t.to_lowercase().contains("cast")
                        || t.contains("?"))),
            "Expected Shield prompt in output, got: {:?}",
            output.text
        );
    }
    ...
    fn test_shield_reaction_not_offered_when_no_slots() {
        ...
        assert!(
            ...
            "Shield should not be offered with no spell slots, got: {:?}",
            output.text
        );
    }
    ...
    fn test_shield_reaction_yes_consumes_slot_and_reaction() {
        ...
        assert!(
            combat.pending_reaction.is_none(),
            "Pending reaction should be cleared after response, got: {:?}",
            output.text
        );
        assert_eq!(
            new_state.character.spell_slots_remaining[0],
            slots_before - 1,
            "Shield should consume a first-level slot"
        );
        // The output must confirm Shield was cast.
        assert!(
            output.text.iter().any(|t| t.contains("Shield")),
        ...
        assert!(
            combat.is_player_turn(),
            "Control should return to player after reaction"
        );
    }
    ...
    fn test_shield_reaction_no_declines_and_resolves_attack() {
        ...
        assert!(
            combat.pending_reaction.is_none(),
            "Pending reaction should be cleared after decline, got: {:?}",
            output.text
        );
        assert_eq!(
            new_state.character.spell_slots_remaining[0], slots_before,
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("decline")),
            "Expected decline narration, got: {:?}",
            output.text
        );
        assert!(
            combat.is_player_turn(),
    ...
    fn test_followup_reaction_prompt_does_not_interleave_with_player_turn_text() {
        ...
        assert!(
            ...
            "Expected a follow-up Shield prompt for the second NPC, got state {:?} with text {:?}",
            combat.pending_reaction,
            output.text
        );
        assert!(
            output
                .text
                .iter()
                .any(|t| t.contains("Second Goblin") && t.to_lowercase().contains("shield")),
            "Expected the follow-up Shield prompt in output, got: {:?}",
            output.text
        );
        assert!(
            !output.text.iter().any(|t| t.contains("Your turn!")),
            "Follow-up reaction prompt must not be mixed with player-turn messaging. Got: {:?}",
            output.text
        );
        assert!(
            !output.text.iter().any(|t| t.starts_with("[HP:")),
            "Follow-up reaction prompt must not include the active-turn HP header. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_cast_shield_on_player_turn_is_rejected_as_reaction_only() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("reaction")),
            "Expected reaction-only narration, got: {:?}",
            output.text
        );
    }
    ...
    fn test_opportunity_attack_prompt_fires_when_npc_leaves_reach() {
        ...
        assert!(
            output.text.iter().any(|t| t.contains("Shortbow")),
            "NPC should have used ranged attack (shortbow), got: {:?}",
            output.text
        );
    }
    ...
    fn test_reaction_yes_no_without_pending_falls_through() {
        ...
        assert!(
            combat.is_player_turn(),
            "Yes with no pending should not consume turn"
        );
        assert!(!combat.action_used);
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("nothing")
                    || t.to_lowercase().contains("unknown")
                    || t.to_lowercase().contains("no pending")
                    || t.to_lowercase().contains("not")),
        ...
        state.world.items.insert(
            ...
            state::Item {
                ...
                item_type: state::ItemType::Weapon {
                    ...
                    damage_die: 4,
                    damage_type: state::DamageType::Piercing,
                    properties: crate::equipment::FINESSE
                        | crate::equipment::LIGHT
                        | crate::equipment::THROWN,
                    category: state::WeaponCategory::Simple,
                    versatile_die: 0,
                    range_normal: 20,
                    range_long: 60,
    ...
    fn test_offhand_attack_requires_main_hand_attack_first() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("main-hand")
                    || t.to_lowercase().contains("main hand")
                    || t.to_lowercase().contains("attack action")),
            "Expected main-hand-required narration, got: {:?}",
            output.text
        );
    }
    ...
    fn test_offhand_attack_consumes_bonus_action_after_attack() {
        ...
        assert!(
            combat.bonus_action_used,
            "Off-hand attack should consume the bonus action. Got: {:?}",
            output.text
        );
        assert!(
            combat.action_used,
    ...
    fn test_offhand_attack_requires_light_offhand_weapon() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("light")),
            "Expected 'light' mention in rejection text, got: {:?}",
            output.text
        );
    }
    ...
    fn test_offhand_attack_requires_offhand_weapon_equipped() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("off hand")
                    || t.to_lowercase().contains("off-hand")),
            "Expected off-hand requirement text, got: {:?}",
            output.text
        );
    }
    ...
    fn test_bonus_dash_grants_movement_and_consumes_bonus_action() {
        ...
        assert!(
            combat.bonus_action_used,
            "BonusDash should consume the bonus action"
        );
        assert!(
            !combat.action_used,
            "BonusDash should NOT consume the action"
        );
        assert_eq!(
            combat.player_movement_remaining,
            baseline_movement + speed,
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("dash")),
            "Expected dash narration, got: {:?}",
            output.text
        );
    }
    ...
    fn test_defeat_state_blocks_regular_commands() {
        ...
        assert!(
            output.text.iter().any(|t| t.contains("GAME OVER")),
            "Expected GAME OVER status. Got: {:?}",
            output.text
        );
        assert!(
            output
                .text
                .iter()
                .any(|t| t.contains("Load a previous save")),
            "Expected recovery options in output. Got: {:?}",
            output.text
        );
    ...
    fn test_end_combat_defeat_mentions_recovery_options() {
        ...
        assert!(lines
            .iter()
            .any(|line| line.contains("Load a previous save")));
    }
    ...
    fn test_new_game_command_on_death_screen() {
        ...
        assert!(
            !output.text.iter().any(|t| t.contains("GAME OVER")),
            "Expected new game, not GAME OVER. Got: {:?}",
            output.text
        );
        assert!(
            output.text.iter().any(|t| t.contains("Choose your race")),
            "Expected character creation prompt. Got: {:?}",
            output.text
        );
    ...
    fn test_game_over_hint_mentions_new_game() {
        ...
        assert!(
            output.text.iter().any(|t| t.contains("new game")),
            "GAME OVER text should mention 'new game'. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_hostile_npcs_get_combat_stats() {
        ...
        for npc in &hostile_npcs {
            assert!(
                ...
                "Hostile NPC '{}' should have combat stats",
                npc.name
            );
        }
    }

    fn give_consumable_to_player(
        state: &mut GameState,
        name: &str,
        description: &str,
        effect: &str,
        ...
        let item = state::Item {
            ...
            item_type: state::ItemType::Consumable {
    ...
    fn test_use_healing_potion_restores_hp() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.contains("HP") || t.contains("heal") || t.contains("hp")),
            "Should mention HP in output. Got: {:?}",
            output.text
        );
        // Item should be consumed
        let new_state: GameState = serde_json::from_str(&output.state_json).unwrap();
        assert!(
            !new_state.character.inventory.contains(&1000),
            "Healing Potion should be removed from inventory"
        );
        assert!(
            !new_state.world.items.contains_key(&1000),
    ...
    fn test_use_torch_dark_to_dim() {
        ...
        let new_state: GameState = serde_json::from_str(&output.state_json).unwrap();
        assert_eq!(
            new_state.world.locations[&loc_id].light_level,
            state::LightLevel::Dim,
    ...
    fn test_use_torch_already_bright() {
        ...
        assert!(
            output.text.iter().any(
                |t| t.to_lowercase().contains("bright") || t.to_lowercase().contains("already")
            ),
            "Should mention already bright. Got: {:?}",
            output.text
        );
        // Still consumed
        let new_state: GameState = serde_json::from_str(&output.state_json).unwrap();
    ...
    fn test_use_rations_nourish() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("nourish")
                    || t.to_lowercase().contains("food")
                    || t.to_lowercase().contains("eat")),
            "Should mention nourishment. Got: {:?}",
            output.text
        );
        let new_state: GameState = serde_json::from_str(&output.state_json).unwrap();
        assert!(!new_state.character.inventory.contains(&1000));
    ...
    fn test_use_non_consumable_item() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("can't use")),
            "Should say can't use. Got: {:?}",
            output.text
        );
    ...
    fn test_objective_command_shows_quest_log_with_objectives() {
        ...
        state
            .progress
            .objective_triggers
            .push(state::ObjectiveType::DefeatNpc(0));

        let state_json = serde_json::to_string(&state).unwrap();
        let output = process_input(&state_json, "objective");
        let text = output.text.join("\n");
        assert!(
            text.contains("=== QUEST LOG ==="),
            "Should show quest log header: {}",
            text
        );
        assert!(
            text.contains("[ ]"),
            "Incomplete objective should show [ ]: {}",
            text
        );
        assert!(
            text.contains("Defeat the Boss"),
            "Should show objective title: {}",
            text
        );
        assert!(
            text.contains("Slay the fearsome enemy"),
    ...
    fn test_objective_command_shows_completed_marker() {
        ...
        assert!(
            text.contains("[X]"),
            "Completed objective should show [X]: {}",
            text
        );
    }
    ...
    fn test_defeat_boss_npc_completes_objective() {
        ...
        state
            .progress
            .objective_triggers
            .push(state::ObjectiveType::DefeatNpc(boss_id));

        let lines = end_combat(&mut state, true);

        assert!(
            state.progress.objectives[0].completed,
            "DefeatNpc objective should be marked complete after combat victory"
        );
        assert!(
            lines
                .iter()
                .any(|l| l.contains("Objective complete") && l.contains("Defeat Boss Enemy")),
            "Should announce objective completion: {:?}",
            lines
        );
    }
    ...
    fn test_take_artifact_completes_find_item_objective() {
        ...
        state
            .progress
            .objective_triggers
            .push(state::ObjectiveType::FindItem(artifact_id));
        ...
        let new_state: GameState = serde_json::from_str(&output.state_json).unwrap();
        assert!(
            new_state.progress.objectives[0].completed,
        ...
        assert!(
            output
                .text
                .iter()
                .any(|l| l.contains("Objective complete") && l.contains("Ancient Gem")),
            "Should announce objective completion: {:?}",
            output.text
        );
        // Quest XP bonus is awarded on FindItem completion.
        assert_eq!(
            new_state.character.xp,
            leveling::OBJECTIVE_XP_REWARD,
    ...
    fn test_take_all_completes_find_item_objective_for_target_item() {
        ...
        assert!(
            output
                ...
                .iter()
                .any(|l| l.contains("Objective complete") && l.contains("Ancient Gem")),
            "Should announce objective completion after take all: {:?}",
            output.text
        );
    }
    ...
    fn test_victory_phase_blocks_exploration_commands() {
        ...
        assert!(
            ...
            "Should show victory message, got: {:?}",
            output.text
        );
        ...
        let output = process_input(&state_json, "help");
        assert!(
            output.text.iter().any(|t| t.contains("Commands")),
            "Help should still work in victory phase: {:?}",
            output.text
        );
        ...
        let output = process_input(&state_json, "quest");
        assert!(
            output.text.iter().any(|t| t.contains("QUEST LOG")),
            "Quest log should still work in victory phase: {:?}",
            output.text
        );
    }
    ...
    fn test_victory_phase_check_returns_not_available_message() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.contains("not available") || t.contains("Skill checks")),
            "check in victory phase should say skill checks not available, got: {:?}",
            output.text
        );
        assert!(
            !output.text.iter().any(|t| t.contains("VICTORY")),
            "check in victory phase should NOT show victory banner, got: {:?}",
            output.text
        );
    }
    ...
    fn test_all_objectives_complete_triggers_victory_phase() {
        ...
        assert!(
            lines.iter().any(|l| l.contains("CONGRATULATIONS")),
    ...
    fn test_objective_command_fallback_for_old_saves() {
        ...
        assert!(
            ...
            "Should show something for old saves"
        );
    }
    ...
    fn test_map_command_lists_discovered_locations_with_current_marker() {
        ...
        assert!(
            output.text.iter().any(|t| t.contains("=== MAP ===")),
            "{:?}",
            output.text
        );
        assert!(
    ...
    fn test_world_generation_seeds_objectives() {
        ...
        assert!(matches!(state.game_phase, GamePhase::Exploration));
        assert!(
            !state.progress.objectives.is_empty(),
            "Should have at least one objective after world generation"
        );
        assert_eq!(
            state.progress.objectives.len(),
            state.progress.objective_triggers.len(),
        ...
        match &state.progress.objective_triggers[0] {
            state::ObjectiveType::DefeatNpc(npc_id) => {
                assert!(
                    state.world.npcs.contains_key(npc_id),
                    "DefeatNpc objective should reference an existing NPC"
                );
                let npc = &state.world.npcs[npc_id];
                assert!(
                    obj.title.contains(&npc.name),
                    "Objective title should contain NPC name: {} not in {}",
                    npc.name,
                    obj.title
                );
            }
            state::ObjectiveType::FindItem(item_id) => {
                assert!(
                    state.world.items.contains_key(item_id),
    ...
    fn test_trap_trigger_failure_applies_damage() {
        ...
        assert!(
            new_state.character.current_hp < start_hp,
        ...
        let all_text = output.text.join("\n");
        assert!(
            all_text.contains("You take 4 damage!"),
            "Output should mention damage taken. Got: {}",
            all_text
        );
    }
    ...
    fn test_look_excludes_dead_npcs() {
        ...
        assert!(
            !all_text.contains("Dead Goblin"),
            "Dead NPC should not appear in look output. Got: {}",
            all_text
        );
        assert!(
            all_text.contains("Friendly Merchant"),
            "Living friendly NPC should appear in look output. Got: {}",
            all_text
        );
    }
    ...
    fn test_spells_command_wizard_shows_known_spells() {
        ...
        assert!(
            all_text.contains("Known Spells"),
            "Should have header. Got: {}",
            all_text
        );
        assert!(
            all_text.contains("Cantrips (at will)"),
            "Should list cantrips. Got: {}",
            all_text
        );
        assert!(
            all_text.contains("Fire Bolt"),
            "Should list Fire Bolt. Got: {}",
            all_text
        );
        assert!(
            all_text.contains("Prestidigitation"),
            "Should list Prestidigitation. Got: {}",
            all_text
        );
        assert!(
            all_text.contains("Level 1 Spells"),
            "Should list level 1 spells. Got: {}",
            all_text
        );
        assert!(
            all_text.contains("Magic Missile"),
            "Should list Magic Missile. Got: {}",
            all_text
        );
        assert!(
            all_text.contains("Burning Hands"),
            "Should list Burning Hands. Got: {}",
            all_text
        );
        assert!(
            all_text.contains("Sleep"),
            "Should list Sleep. Got: {}",
            all_text
        );
        assert!(
            all_text.contains("Shield"),
            "Should list Shield. Got: {}",
            all_text
        );
    }
    ...
    fn test_spells_command_wizard_shows_spell_slots() {
        ...
        assert!(
            all_text.contains("Spell Slots"),
            "Should show spell slots section. Got: {}",
            all_text
        );
        assert!(
            all_text.contains("Level 1: 2/2"),
            "Should show 2/2 level 1 slots. Got: {}",
            all_text
        );
    }
    ...
    fn test_spells_command_non_wizard_no_spells() {
        ...
        assert!(
            all_text.contains("You don't know any spells"),
            "Non-wizard should get no-spells message. Got: {}",
            all_text
        );
    }
    ...
    fn test_spells_command_does_not_mutate_state() {
        ...
        assert_eq!(
            new_state.character.known_spells,
            state.character.known_spells
        );
    }
    ...
    fn test_short_rest_command_through_process_input() {
        ...
        assert!(
            ...
            "Expected short rest narration, got: {}",
            text
        );
    ...
    fn end_combat_awards_xp_for_dead_hostile() {
        ...
        assert!(
            lines.iter().any(|l| l.contains("50 XP")),
            "Lines: {:?}",
            lines
        );
    }
    ...
    fn defeat_objective_completion_awards_quest_xp() {
        ...
        if let Some(c) = state.active_combat.as_mut() {
            c.initiative_order
                .push((crate::combat::Combatant::Npc(boss_id), 5));
        }
    ...
    fn tough_feat_grants_extra_hp_on_level_up() {
        ...
        assert_eq!(
            hp_gain,
            base_gain_without_tough + 2,
            "Tough should add +2 HP per level-up"
        );
    }
    ...
    fn no_tough_feat_no_extra_hp_on_level_up() {
        ...
        assert_eq!(
            hp_gain, expected_base,
            "No feat — only base HP gain expected"
        );
    }
    ...
    fn tough_in_origin_feat_also_grants_level_hp_bonus() {
        ...
        assert_eq!(
            hp_gain,
            expected_base + 2,
            "Origin feat Tough should also grant +2 HP/level"
        );
    }
    ...
    fn choose_asi_plus2_str_raises_by_2_and_consumes_credit() {
        ...
        assert_eq!(
            state.character.ability_scores[&Ability::Strength],
            17,
            "STR should be 17 after +2 from 15. Got: {:?}",
            result
        );
    ...
    fn choose_asi_plus1_str_dex_raises_each_and_consumes_credit() {
        ...
        state
            .character
            .ability_scores
            .insert(Ability::Dexterity, 13);
        let result = handle_choose_asi(&mut state, "+1 STR DEX");
        assert_eq!(
            state.character.ability_scores[&Ability::Strength],
            15,
            "STR should be 15. Got: {:?}",
            result
        );
        assert_eq!(
            state.character.ability_scores[&Ability::Dexterity],
            14,
            "DEX should be 14. Got: {:?}",
            result
        );
        assert_eq!(state.character.asi_credits, 0);
    }
    ...
    fn choose_asi_archery_rejected_for_non_fighter() {
        ...
        assert!(
            result.iter().any(|l| l.contains("Only Fighters")),
            "Expected fighting-style gate error. Got: {:?}",
            result
        );
    ...
    fn choose_asi_origin_feat_rejected() {
        ...
        assert!(
            ...
            "Alert (origin feat) should be rejected in ASI phase. Got: {:?}",
            result
        );
        assert_eq!(
            state.character.asi_credits, 1,
    ...
    fn choose_asi_cancel_defers_without_spending() {
        ...
        assert!(
            result
                .iter()
                .any(|l| l.contains("aside") || l.contains("cancel") || l.contains("defer")),
            "Cancel should defer. Got: {:?}",
            result
        );
        assert_eq!(
            state.character.asi_credits, 1,
            "Credit must not be consumed on cancel"
        );
        assert!(matches!(state.game_phase, GamePhase::Exploration));
    }
    ...
    fn test_ritual_cast_of_ritual_spell_succeeds_without_slot() {
        ...
        assert_eq!(
            new_state.character.spell_slots_remaining, slots_before,
    ...
    fn test_mage_armor_in_exploration_raises_ac_and_uses_slot() {
        ...
        let text = output.text.join(" ");
        assert!(
            text.to_lowercase().contains("base ac is now 13")
                || text.to_lowercase().contains("protective weave"),
            "Expected mage armor narration. Got: {:?}",
            output.text
        );
        ...
        assert!(
            ac_after > ac_before,
            "Mage Armor should raise AC. Before: {}, after: {}",
            ac_before,
            ac_after
        );
        assert_eq!(
        ...
        assert_eq!(
            new_state.character.class_features.mage_armor_until_minutes,
            Some(8 * 60),
    ...
    fn test_ritual_cast_of_non_ritual_spell_is_rejected() {
        ...
        let text = output.text.join("\n");
        assert!(
            text.contains("doesn't have the Ritual tag") || text.to_lowercase().contains("ritual"),
    ...
    fn test_concentration_save_helper_breaks_on_failure() {
        ...
        assert!(
            !lines.is_empty(),
            "Helper should emit at least one concentration line"
        );
        ...
        assert!(
            broken || held,
            "Helper must leave state in a consistent concentration state"
        );
    }
    ...
    fn test_wizard_keeps_int_based_spellcasting() {
        ...
        assert!(
            ...
            "Wizard should know Fire Bolt. Got:\n{}",
            text
        );
    }
    ...
    fn test_cleric_can_see_known_spells_via_spells_command() {
        ...
        let text = output.text.join("\n");
        assert!(
            text.contains("Known Spells"),
            "Cleric should have known spells. Got:\n{}",
            text
        );
        assert!(
            text.to_lowercase().contains("cure wounds")
    ...
    fn test_fighter_still_has_no_spells() {
        ...
        let text = output.text.join("\n");
        assert!(
            text.contains("You don't know any spells"),
    ...
    fn give_wondrous_to_player(
        state: &mut GameState,
        name: &str,
        effect: equipment::magic::WondrousEffect,
        ...
        let item = state::Item {
            ...
            name: name.to_string(),
            description: String::new(),
            item_type: state::ItemType::Wondrous {
                effect,
                rarity,
                requires_attunement,
            },
    ...
    fn give_magic_weapon_to_player(
        state: &mut GameState,
        name: &str,
        base: &str,
        attack_bonus: i32,
    ...
    fn test_attune_to_wondrous_item_succeeds() {
        ...
        let mut state = create_test_exploration_state();
        let id = give_wondrous_to_player(
            &mut state,
            "Cloak of Protection",
            WondrousEffect::CloakOfProtection,
            Rarity::Uncommon,
            true,
        );
        ...
        let new_state: GameState = serde_json::from_str(&output.state_json).unwrap();
        assert!(
            new_state.character.attuned_items.contains(&id),
            "Expected attuned_items to contain id {}, got: {:?}",
            id,
            new_state.character.attuned_items
        );
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("attune")
                    || t.to_lowercase().contains("resonate")),
            "Expected attunement narration. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_attune_cap_blocks_fourth_attunement() {
        ...
        let b = give_wondrous_to_player(
            &mut state,
            "Ring of Protection",
            WondrousEffect::RingOfProtection,
            Rarity::Rare,
            true,
        );
        let c = give_wondrous_to_player(
            &mut state,
            "Gauntlets of Ogre Power",
            WondrousEffect::GauntletsOfOgrePower,
            Rarity::Uncommon,
            true,
        );
        // Fourth item — adding should fail.
        let d = give_wondrous_to_player(
            &mut state,
            "Belt of Giant Strength",
            WondrousEffect::BeltOfGiantStrength(21),
            Rarity::Rare,
            true,
        );
        ...
        let new_state: GameState = serde_json::from_str(&output.state_json).unwrap();
        assert!(
            !new_state.character.attuned_items.contains(&d),
            "Fourth attunement must not be added. Got: {:?}",
            new_state.character.attuned_items
        );
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("already attuned")
                    || t.to_lowercase().contains("unattune")
                    || t.to_lowercase().contains("max")),
            "Expected cap-exceeded narration. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_attune_non_attunement_item_rejects() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("does not require")),
            "Expected 'does not require attunement' narration. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_attune_non_magic_item_rejects() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("not a magic item")),
            "Expected 'not a magic item' narration. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_unattune_removes_from_list() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("release")),
            "Expected release narration. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_list_attunements_empty() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("not attuned") && t.contains("0")),
            "Expected 'not attuned' narration. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_list_attunements_shows_items() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.contains("Cloak of Protection")),
            "Expected cloak listed. Got: {:?}",
            output.text
        );
        assert!(
            output
    ...
    fn test_use_potion_of_healing_restores_hp() {
        ...
        // Potion is consumed.
        assert!(
            !new_state.character.inventory.contains(&id),
            "Potion should be removed from inventory. Got: {:?}",
            new_state.character.inventory
        );
        assert!(
            !new_state.world.items.contains_key(&id),
            "Potion should be removed from world items"
        );
        // HP went up (2d4+2 is 4..=10).
        assert!(
            new_state.character.current_hp > 10,
            "Expected HP to increase. Got {}",
            new_state.character.current_hp
        );
        assert!(
            new_state.character.current_hp <= 30,
            "Must not exceed max_hp"
        );
        // Narration mentions healing.
        assert!(
            output.text.iter().any(|t| t.to_lowercase().contains("heal")
    ...
    fn test_use_flavor_potion_is_consumed_with_narration() {
        ...
        assert!(
            !new_state.character.inventory.contains(&id),
            "Flavor potion should still be consumed. Got: {:?}",
            new_state.character.inventory
        );
        assert!(!output.text.is_empty());
    }
    ...
    fn test_use_wand_decrements_charges() {
        ...
        let remaining = new_state
            .world
            .items
            .get(&id)
            .and_then(|i| i.charges_remaining)
            .expect("wand still exists");
        assert_eq!(
            remaining, 6,
            "One charge should be spent. Got {}",
            remaining
        );
    ...
    fn test_use_wand_with_zero_charges_fails() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("no charges")
                    || t.to_lowercase().contains("spent")
                    || t.to_lowercase().contains("depleted")),
            "Expected no-charges narration. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_use_wand_requires_attunement_when_gated() {
        ...
        assert!(
            output
                ...
                .iter()
                .any(|t| t.to_lowercase().contains("attune")),
            "Expected attunement-required narration. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_use_scroll_by_spellcaster_consumes_scroll() {
        ...
        assert!(
            !new_state.character.inventory.contains(&id),
            "Scroll should be consumed by a spellcaster. Got: {:?}",
            new_state.character.inventory
        );
        assert!(!output.text.is_empty());
    }
    ...
    fn test_use_scroll_by_nonspellcaster_rolls_arcana_check() {
        ...
        assert!(
            full.contains("arcana") || full.contains("dc 10") || full.contains("check"),
            "Expected DC 10 Arcana-check narration. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_world_magic_item_rarity_weighted_common_dominates() {
        ...
        assert!(
            common > legendary,
            "Expected Common ({}) > Legendary ({}) over 5000 spawns",
            common,
            legendary
        );
    }
    ...
    fn test_attack_with_sap_mastery_marks_target() {
        ...
        for seed in 0..40u64 {
            ...
            let output = process_input(&state_json, "attack test goblin");
            let new_state: GameState = serde_json::from_str(&output.state_json).unwrap();
            let Some(combat) = new_state.active_combat.as_ref() else {
                continue;
            };
            if combat.sap_targets.contains(&100) {
                // Success: hit landed and Sap mark was set.
                assert!(
                    output.text.iter().any(|t| t.contains("Sap")),
                    "Expected Sap narration, got: {:?}",
                    output.text
                );
                return;
            }
    ...
    fn test_graze_on_miss_deals_damage_end_to_end() {
        ...
        for seed in 0..40u64 {
            ...
            let output = process_input(&state_json, "attack test goblin");
            let new_state: GameState = serde_json::from_str(&output.state_json).unwrap();
            let new_hp = new_state.world.npcs[&100]
                .combat_stats
                .as_ref()
                .unwrap()
                .current_hp;
            ...
            if missed && new_hp < start_hp {
                ...
                assert!(
                    output.text.iter().any(|t| t.contains("Graze")),
                    "Expected Graze narration on miss-with-damage, got: {:?}",
                    output.text
                );
                return;
            }
    ...
    fn test_attack_with_push_mastery_shoves_target() {
        ...
        for seed in 0..40u64 {
            ...
            if dist == 15 {
                // Push fired: 5 -> 15.
                assert!(
                    output.text.iter().any(|t| t.contains("Push")),
                    "Expected Push narration, got: {:?}",
                    output.text
                );
                return;
            }
    ...
    fn seed_objectives_then_defeat_boss_completes_objective() {
        ...
        assert!(
            ...
            boss_id,
            state.world.npcs.keys().collect::<Vec<_>>()
        );

        let boss_npc = boss_npc.unwrap();
        assert!(
            boss_npc.combat_stats.is_some(),
        ...
        assert!(
            state.progress.objectives[0].completed,
            "DefeatNpc objective should be marked complete after defeating the boss. Lines: {:?}",
            lines
        );
        assert!(
            lines.iter().any(|l| l.contains("Objective complete")),
    ...
    fn process_input_navigate_to_boss_room_and_kill_completes_objective() {
        ...
        let hostiles_in_boss_room: Vec<u32> = state
            .world
            .locations
            .get(&boss_location)
            .map(|loc| {
                loc.npcs
                    ...
                    .filter(|&&id| {
                        state
                            ...
                            .get(&id)
                            .map(|n| {
                                n.disposition == state::Disposition::Hostile
                                    && n.combat_stats.is_some()
                            })
                            .unwrap_or(false)
                    })
                    .copied()
            ...
            if text.contains("VICTORY") {
                assert!(
                    ...
                    "DefeatNpc objective should be complete after killing boss via process_input. \
                     Boss id: {}, boss_name: {}, text: {}",
                    boss_id, boss_name, text
                );
                assert!(
                    text.contains("Objective complete"),
                    "Should announce objective completion. Text: {}",
                    text
                );
                found_seed = true;
                break;
    ...
    fn test_combat_second_equip_costs_utilize_action() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("utilize")
                    || t.to_lowercase().contains("free object interaction")),
            "Output should mention Utilize action or free object interaction. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_combat_second_equip_blocked_when_action_used() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.contains("already used") || t.contains("can't")),
            "Should be blocked when both free interaction and action are used. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_smoke_end_to_end_public_api() {
        ...
        assert!(
            !output.state_json.is_empty(),
            "new_game: state_json must not be empty"
        );
        ...
        assert!(
            !state.world.locations.is_empty(),
            "World must have been generated"
        );
        assert!(
            !state.character.inventory.is_empty(),
        ...
        assert!(
            look_out.text.iter().any(|l| !l.is_empty()),
        ...
        assert!(
            help_out.text.iter().any(|l| !l.is_empty()),
    ...
    fn test_use_tool_unknown_tool_returns_error() {
        ...
        assert!(
            combined.contains("don't have")
                || combined.contains("not found")
                || combined.contains("no tool"),
            "Expected an error for an unknown tool, got: {:?}",
            out.text
        );
    }
    ...
    fn test_ammo_opt_in_allows_attack_when_key_absent() {
        ...
        let state = create_test_exploration_state();
        assert!(
            state.character.ammo.is_empty(),
            "ammo map should be empty for opt-out"
        );
    ...
    fn test_combat_turn_prompt_contains_hp_header() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|l| l.starts_with("[HP:") && l.contains("|")),
    ...
    fn test_new_game_mid_exploration_shows_confirmation() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|l| l.contains("Start a new game") || l.contains("lose")),
            "Should show confirmation prompt. Got: {:?}",
            output.text
        );
        ...
        assert!(
            ...
            "Phase should still be Exploration after first 'new game'. Got: {:?}",
            new_state.game_phase
        );
        assert!(
            new_state.pending_new_game_confirm,
    ...
    fn test_new_game_confirmation_yes_reinitializes() {
        ...
        assert!(
            ...
            "Game should reinitialize to CharacterCreation. Got: {:?}",
            new_state.game_phase
        );
    }
    ...
    fn test_inventory_groups_identical_items() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|l| l.contains("Javelin") && l.contains("(x3)")),
            "Inventory should show 'Javelin (x3)'. Got: {:?}",
            output.text
        );
        ...
        assert_eq!(
            javelin_count, 1,
            "Javelins should be grouped into one line. Got: {:?}",
            output.text
        );
    }
    ...
    fn test_combat_potion_allows_main_action_attack_after_drink() {
        ...
        assert!(
            !combat.action_used,
            "Main action should still be available after drinking"
        );
    }
    ...
    fn test_potion_narration_mentions_recover() {
        ...
        assert!(
            output
                .text
                .iter()
                .any(|t| t.to_lowercase().contains("recover") || t.to_lowercase().contains("hp")),
    ...
    fn test_wizard_spellbook_prompt_lists_level1_wizard_spells() {
        ...
        let combined = out.text.join(" ").to_lowercase();
        assert!(
            combined.contains("spellbook") || combined.contains("spell"),
            "Expected spell selection prompt, got: {:?}",
            out.text
        );
        assert!(
            combined.contains("magic missile") || combined.contains("burning hands"),
            "Expected level-1 wizard spells listed, got: {:?}",
            out.text
        );
    }
    ...
    fn test_wizard_spellbook_accepts_6_valid_selections() {
        ...
        assert_eq!(
            state.character.known_spells.len(),
            8,
    ...
    fn test_wizard_spellbook_rejects_wrong_count() {
        ...
        assert!(
            combined.contains("6"),
            "Error should mention requiring 6 spells"
        );
    }
    ...
    fn test_wizard_prepared_spells_prompt_shows_spellbook() {
        ...
        assert!(
            combined.contains("prepared") || combined.contains("prepare"),
            "Should show prepare prompt, got: {:?}",
            after_book.text
        );
    }
    ...
    fn test_wizard_prepared_spells_accepts_int_mod_plus_level_count() {
        ...
        assert!(
            ...
            "Should have left ChooseWizardPreparedSpells, got {:?}",
            state.game_phase
        );
    }
    ...
    fn test_wizard_prepared_spells_rejects_wrong_count() {
        ...
        assert!(
            combined.contains("4"),
            "Error should mention requiring 4 spells"
        );
    }
    ...
    fn test_wizard_prepared_spells_populate_class_features() {
        ...
        assert_eq!(
            state.character.class_features.prepared_spells.len(),
            4,
    ...
    fn test_new_game_from_state_valid_json_returns_ready_output() {
        ...
        assert!(
            ...
            "Expected state_changed == true for valid state"
        );
        assert!(
            !output.state_json.is_empty(),
            "Expected non-empty state_json"
        );
